...
void kill_processes(t_philo *mass, int flag, pid_t ret)
{
	int i;
	int count;
	...
	if (!flag)
	{
		while (i < count)
		{
			if (WEXITSTATUS(g_status) != i)
				kill(mass[i].ret, SIGKILL);
			i++;
		}
	}
	else
...
int init_threads(t_philo *philo_mass, t_input_data *input_data)
{
	...
	if (input_data->num_of_t_to_eat < INT_MAX)
	{
		...
		kill_processes(philo_mass, 1, ret);
	}
	else
	{
		waitpid(-1, &g_status, 0);
		kill_processes(philo_mass, 0, 0);
	}
	return (0);
}
